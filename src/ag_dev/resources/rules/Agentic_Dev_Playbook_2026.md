...
> **日期**: 2026-01-18
> **版本**: 2.5 (ERP 實戰強化版)
> **範圍**: 進階人機協作、長上下文管理、安全與衛生規範、認知治理、結構秩序、脈絡完整性、自我批判、持續迭代、業務系統治理
...
## 🏢 第十七部：業務系統 Agent 分工 (The Triad for Business Systems)
> **新增於**: 2026-01-18 (V2.5)
> **來源**: Chen Zhen Fang ERP 實戰經驗

對於 ERP、LINE Bot、電商等業務系統，建議採用以下 Agent 分工：

| Agent | 角色 | 職責 |
| :--- | :--- | :--- |
| **Agent D: Data** | 資料層 | DB Schema, Migrations, Seeding, Query Optimization |
| **Agent E: Integration** | 整合層 | External APIs (LINE, Payment, Shipping), Webhooks, OAuth |
| **Agent F: Domain** | 業務層 | Business Rules, Accounting, NLP, Cultural/Religious Logic |

---

## 🌐 第十八部：Browser Subagent 紀律 (Protocol 17)
> **新增於**: 2026-01-18 (V2.5)

### 17.1 何時使用 Browser Subagent
*   **憑證提取**: 從 Web Console 抓取 API Token、Secret (e.g., LINE, ngrok, AWS)
*   **配置設定**: 設定 Webhook URL、OAuth Callback 等需要 UI 操作的任務
*   **視覺驗證**: 驗證 Webhook 連線、截圖 UI 狀態

### 17.2 何時不使用
*   **靜態文件**: 使用 `read_url_content` 讀取 API 文件
*   **可程式化 API**: 能用 CLI/Code 完成的不用瀏覽器

### 17.3 錄製政策
*   所有 Subagent 動作自動錄製為 `.webp`
*   存放於 `.system_generated/` 以供審計

---

## 📦 第十九部：Skill 自動結晶協議 (Protocol 18)
> **新增於**: 2026-01-18 (V2.5)

### 18.1 觸發條件
當以下情況發生時，**必須** 建立新 Skill 文件：
1.  **可重用模式**: 出現可抽象的邏輯 (e.g., NLP Parser, API Wrapper)
2.  **重複使用 3 次以上**: 相同程式碼結構出現 3 次
3.  **業務邏輯文件化**: 需要讓非技術人員理解的業務規則
...
## 🔐 第二十部：憑證生命週期管理 (Protocol 19)
> **新增於**: 2026-01-18 (V2.5)

### 19.1 佔位符慣例
*   使用 `YOUR_XXX_HERE_PLEASE_UPDATE` 作為標記
*   系統首次運行時 **必須** 檢查這些佔位符並中止執行

### 19.2 Browser 提取流程
1.  檢查 `.env` 是否有佔位符
2.  若有，調用 Browser Subagent 從 Console 提取
3.  更新 `.env` 並立即加入 `.gitignore`
...
## 📖 第二十一部：領域知識治理 (Protocol 20)
> **新增於**: 2026-01-18 (V2.5)

### 20.1 靜態資料來源
*   必須存放於 `data/` 目錄 (JSON/CSV)
*   必須版本控制，禁止動態生成關鍵數據

### 20.2 確定性優先原則
對於 **零容錯領域** (宗教、醫療、法律)：
*   **禁止** 使用 LLM 生成答案 (Hallucination Risk)
*   **必須** 使用經驗證的函式庫 (e.g., `lunar-python`) 或人工審核資料庫
...
### 22.2 禁止相對路徑
*   `sqlite:///data/mydb.db` **嚴禁** (工作目錄變更會導致失敗)

---

## 🔍 第二十三部：自我審查機制 (Protocol 21: Self-Review for Playbook Evolution)
> **新增於**: 2026-01-18 (V2.5)
...
### 21.3 立即修正決策樹 (Fix Now or Defer?) 🆕
發現問題後，使用以下決策樹判斷是否立即修正：
...
### 21.4 反思問題清單 (Retrospective Questions)
在每次對話結束前，自問：
1.  **Why**: 這次任務為什麼成功/失敗？
2.  **What**: 學到了什麼新模式或反模式？
3.  **How**: 如何防止同樣的問題再次發生？
...
### 21.5 Gap Discovery Flow (缺陷發現流程)
當發現 Playbook 未涵蓋的情境時：
1.  **記錄**: 在 `thought` 中標記 `[PLAYBOOK_GAP]`
2.  **分類**: 判斷 P0-P3 等級
3.  **決策**: 使用 21.3 決策樹判斷立即修正或延後
4.  **報告**: 在 `notify_user` 訊息中加入 `### 🔴 Playbook Gap Alert` 區塊
...
## ⚠️ 附錄 E: 多代理架構說明 (Multi-Agent Architecture Clarification)
> **新增於**: 2026-01-18 (V2.5)
> **目的**: 釐清 "Multi-Agent" 的真實含義

### E.1 術語釐清
| 術語 | 含義 | 實際能力 |
| :--- | :--- | :--- |
| **Agent A/B/C** | **概念角色** (Conceptual Roles) | 同一 Main Agent 扮演不同角色 |
| **Browser Subagent** | **真正的子代理** | 唯一可調用的獨立工具 |
| **Federated Model** | **行為模式** | 描述 Agent 在複雜任務中的角色切換 |

### E.2 系統限制
*   ❌ 目前系統 **無法** 由使用者自定義新的 Agent
*   ❌ Playbook 中的 "3 Agents" 是 **邏輯分工**，非物理隔離的進程
*   ✅ 唯一可調用的 Subagent: `browser_subagent` (用於瀏覽器自動化)

### E.3 未來可能性
若系統升級支援真正的多代理架構，建議：
1.  **Skill Agent**: 專門處理特定 Skill (e.g., `skill_db_inspector`)
2.  **Verification Agent**: 專門執行測試與審計
3.  **Research Agent**: 專門進行 Web 研究與文件閱讀

---

> *本文件為 Antigravity Agentic Workflow V2.5 的最高指導原則。*
